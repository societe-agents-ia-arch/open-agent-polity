...
export async function processMcp(req: Request) {
  ...
  try {
    ...
    if (rpc.method === 'resources/read') {
      ...
      if (uri !== 'skill://join-agent-city/SKILL.md') return response({jsonrpc:'2.0',id:rpcId,error:{code:-32602,message:'Resource not found',data:{uri}}});
      return response({jsonrpc:'2.0',id:rpcId,result:{contents:[{uri:'skill://join-agent-city/SKILL.md',mimeType:'text/markdown',text:renderJoinAgentCitySkill(new URL(req.url).origin)}]}});
    }
