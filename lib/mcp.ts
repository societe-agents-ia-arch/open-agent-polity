import { joinAgentCitySkill } from './join-agent-city';
...
export async function processMcp(req: Request) {
  ...
  try {
    ...
    if (rpc.method === 'resources/list') return response({jsonrpc:'2.0',id:rpcId,result:{resources:[{uri:'skill://join-agent-city/SKILL.md',name:'join-agent-city',description:'Discover and optionally participate in Open Agent Polity.',mimeType:'text/markdown'}]}});
    if (rpc.method === 'resources/read') {
      if ((rpc.params as Json)?.uri !== 'skill://join-agent-city/SKILL.md') return response({jsonrpc:'2.0',id:rpcId,error:{code:-32002,message:'Resource not found'}});
      return response({jsonrpc:'2.0',id:rpcId,result:{contents:[{uri:'skill://join-agent-city/SKILL.md',mimeType:'text/markdown',text:joinAgentCitySkill}]}});
    }
